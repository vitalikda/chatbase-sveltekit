...
<header class="p-6 bg-white/5 border-b border-[#363739]">
	<div class="max-w-4xl mx-auto">
		<div class="flex justify-between items-center">
			<p class="inline-flex items-center space-x-3">
				<a href="https://grafbase.com?ref=chatbase" target="_blank" rel="noopener noreferrer">
					<svg
						width="20"
						height="20"
						class="flex-shrink-0 h-6 w-6 fill-current text-white/50 hover:text-white/100 transition"
						viewBox="0 0 41 41"
				...
				<span class="text-white font-bold text-xl">Chatbase</span>
			</p>
			{#if user}
				<div class="flex space-x-1">
					...
					<button
						on:click={() => signOut()}
						class="bg-white/5 rounded h-12 px-6 font-medium text-white border border-transparent"
					>
						Sign out
					</button>
				</div>
			{:else}
				<div class="flex items-center">
					<button
						...
						class="bg-white/5 rounded h-12 px-6 font-medium text-white text-lg border border-transparent inline-flex items-center"
					>
						Sign in with GitHub
					</button>
				</div>
...
{#if user}
	<slot />
{:else}
	<div class="h-full flex items-center justify-center flex-col space-y-2.5">
		<p class="text-lg md:text-2xl lg:text-3xl font-medium text-white">
		...
		<p>
			<a
				...
				class="text-sm text-white/50 transition hover:text-[#4a9c6d]/100"
			>
